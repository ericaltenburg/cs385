/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Eric Altenburg and Anthony Quattrocchi
 * Version     : 1.0
 * Date        : April 22, 2019
...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        // Followed pseudo code from the textbook with some slight modifications
        ...
        while (x != NULL) {
        	...
        	// May have to potentially look at a case where a node being inserted already exists
        	if (z->key() == x->key()) { // The node has already been inserted
        		std::cout << "Warning: Attempt to insert duplicate key '" << z->key() << "'." <<std::endl;
        		return; // Hops out of the insert and finishes
        	}
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
    	if (n != NULL) { // If the node is not null, then it has to be deleted.
    		delete_tree(n->left); // Call left child
    		delete_tree(n->right); // Call right child
    ...
    void insert_fixup(Node<K, V> *z) {
        // Followed pseudo code attained from the textbook
    ...
    int height(Node<K, V> *node) const {
    	if (node == NULL) { // Tree with null root has height -1
    		return -1;
    	} else { // Find the max height between the left and right subtrees
    ...
    size_t leaf_count(Node<K, V> *node) const {
    	if (node == NULL) { // Have reached the child of a child which is NULL
    		return 0;
    	} else if (node->left == NULL && node->right == NULL) { // A child has been found
    		return 1;
    	} else {
    ...
    int diameter(Node<K, V> *node) const {
    	...
    	int heights = 2 + height(node->left) + height(node->right);
    	int temp = std::max(diameter(node->left), diameter(node->right));
    	return std::max(heights, temp);
    }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        // Node is root, start there then traverse down by recursively calling children and decrementing level by 1
    	if (node == NULL) // Reached bottom of the three and the level does not exist
    		return 0;
    	if (level == 0) {// Reached the desired level
    		return 1;
    	} else { // Not on the right level, go down another level through children
    ...
    size_t null_count(Node<K, V> *node) const {
    	if (node == NULL) { // Reached a null node
    		return 1;
    	} else { // A non-null node has been found, continue going down until one is reached
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
    	if (node == NULL) { // If the node is null, then it returns 0 as it wants the non-null nodes
    		return 0;
    	} else { // This is a non-null child, so add the levels through the children
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
    	...
    	if (node != NULL) { // A non-null node has been found, continue on searching
    		return sum_null_levels(node->left, level+1) + sum_null_levels(node->right, level+1);
    	} else { // Found a null node, return the level
    		return level;
    	}
    }
};
